"""Async version of the Crossref source client."""
...
async def search_crossref_async(
    ...
    *,
    ...
    for q in queries:
        ...
        resp = await safe_get_async(client, "https://api.crossref.org/works", params=params)
        if resp is None:
            continue
        try:
            data = resp.json()
        except Exception as exc:
            log.warning("Crossref JSON parse failed for %r: %s", q, exc)
            continue
