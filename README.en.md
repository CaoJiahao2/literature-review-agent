...
📘 **Architecture & design docs (中文)** under [`docs/zh/`](./docs/zh/) — start with [`docs/zh/ARCHITECTURE.md`](./docs/zh/ARCHITECTURE.md) if you're hacking on the project.
...
- ⚡ **Async source fan-out** — every source query is dispatched concurrently with a bounded semaphore.
- 🤖 **LLM client facade** — transparent caching (`LIT_REVIEW_CACHE_DIR`), retry/backoff, token accounting.
- 📊 **Built-in observability** — `--emit-metrics` writes per-node timing, per-source hits, LLM usage, sections sizes; `--emit-state` writes a debuggable state snapshot.
...
# 6. With observability (writes <report>.metrics.json)
...
- **74 offline tests** — state, templates, rank/dedupe, CLI, graph, async runner, LLM client, metrics (~1.8s)
